...
SwissSec includes a dedicated Chrome Extension to bridge the hardware token seamlessly with your daily browsing experience.

* **WebSerial Dashboard:** A secure, AES-GCM encrypted GUI interface to manage the hardware token directly from a browser tab.
* **Context Menu Autofill:** Right-click integration to detect the active domain, auto-generate passwords, and inject retrieved credentials directly into website login forms.
* **Visual Vault Management:** Graphic interfaces to list, create, and wipe FIDO2 passkeys, standard passwords, and TOTP secrets without typing serial commands.
* **Device Administration:** Easily trigger the hardware diagnostic suite, enroll new fingerprints, select post-quantum algorithms, or factory reset the device from a centralized settings tab.
